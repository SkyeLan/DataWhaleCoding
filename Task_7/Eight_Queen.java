...
public class Eight_Queen {
    private final short N = 8;        //使用常量来定义，方便之后解N皇后问题
    private int count = 0;            //结果计数器

    private int putQueenAtRow(short[][] chess, int row) {
        /**
    ...
    private boolean isSafety(short[][] chess, int row, int col) {
        //判断中上、左上、右上是否安全
        int step = 1;
        while (row - step >= 0) {
    ...
    public static void main(String[] args) {
        ...
        int count = new Eight_Queen().putQueenAtRow(chess, 0);
        Date end = new Date();
